package com.kh.semi.reply2.model.dao;
...
public class ReplyDAO {
	...
	public ReplyDAO(){
		String fileName = ReplyDAO.class.getResource("/com/kh/semi/sql/reply2/reply-query.xml").getPath();
		try {
			prop = new Properties();
	...
	public List<Reply> selectList(Connection conn, int parentBoardNo) throws Exception{
		...
		try {
			...
			while(rset.next()) {
				...
				reply.setReplyNo(rset.getInt("TRIP_REPLY"));
				reply.setReplyContent(rset.getString("TRIP_REPLY_CONTENT"));
				reply.setReplyCreateDate(rset.getTimestamp("TRIP_REPLY_DT"));
