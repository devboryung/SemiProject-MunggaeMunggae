...
$(".comphone").on("input", function(){
    ...
    var v1 = $("#comPhone2").val();
    var v2 = $("#comPhone3").val();

    if(!regExp1.test(v1) || !regExp2.test(v2)){
        $("#checkPhone1").text("전화번호가 유효하지 않습니다.")
            .css("color", "red");
        validateCheck2.comPhone2 = false;
    }else{
        $("#checkPhone1").text("유효한 형식의 전화번호 입니다.")
            .css("color", "green");
        validateCheck2.comPhone2 = true;
    }
...
$("#companyName").on("input", function(){
    ...
    var v1 = $("#companyName").val();

    if(!regExp.test(v1)){
        $("#checkCNM").text("업체명 형식이 유효하지 않습니다.").css("color", "red");
        validateCheck2.comName = false;
    } else {
        $("#checkCNM").text("유효한 업체명 형식입니다.").css("color", "green");
        validateCheck2.comName = true;
    }
