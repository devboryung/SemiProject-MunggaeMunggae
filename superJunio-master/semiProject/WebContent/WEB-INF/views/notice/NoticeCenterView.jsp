     ...
     <div class="">
            <div class="">
                 <div class="" style="margin-left:30px;">
                    <h2 style="margin-top:30px;">${notice.noticeTitle}</h2>   
 			    </div>
                    <div class="col-md-12" style="text-align:right; font-size: 0.5em;
					color: gray;">조회수:${notice.noticeReadCount}</div>
       </div>
        </div>  
        <hr>         
         <div class="" style="margin-left:30px;">
                <div class="row">
                    <div class="col-md-8" style="color:gray;font-size: 0.8em;">
                 	       글쓴이 :${notice.memNickName}
                    </div>
                    <div class="col-md-4" style="text-align:right; font-size: 0.5em;
					color: gray;">
			                        작성일 :<fmt:formatDate value="${notice.noticeCreateDt}" pattern="yyyy년MM월dd일HH:mm:ss"/>
                    </div>
            </div>
           ...
           <div class="">
                            ...
                            <div class="carousel-inner">
                              <c:forEach var="file" items="${fList}" varStatus="vs">
                                ...
                                <div class="carousel-item <c:if test="${vs.first}">active</c:if>">
                                    <img class="d-block w-100 boardImg bigImg"  id="${file.notiFileNo}" 
                                    src="${contextPath}/resources/image/center/noticeImg/${file.notifileName}"/>
                                </div>
            ...
            <div id="" style="height: 250px; margin-top:20px;">${notice.noticeContent}</div>
           </div>
              ...
              <c:choose>	
				  <c:when test="${!empty param.sk && !empty param.sv}">
				    ...
				    <c:url var="goToList" value="../centerSearch/notice.do">
				  ...
				  <c:otherwise>
						<c:url var="goToList" value="notice.do">
		             	  <c:param name="cp">${param.cp}</c:param>					 
						</c:url>
				  </c:otherwise>
			</c:choose>
        ...
        <div class="float-right" id="button">
            <button id="" class=" buttons btn" onclick="location.href='${goToList}'">목록으로</button>
            
            
                        
			<c:if test="${!empty loginMember && (notice.memNo == loginMember.memberNo)}">
            ...
            <button type="button" id="updateBtn"class=" btn ml-1 mr-1 buttons">수정</button>
            <button type="button" id="deleteBtn"class=" btn ml-1 mr-1 buttons">삭제</button>
        	</c:if>
        </div>
	...
	swal({
		...
		.then((willDelete) => {
		  if (willDelete) {
			  ...
			  location.href = "deleteNotice.do?no=${notice.noticeNo}";			  
...
$("#updateBtn").on("click",function(){
	 ...
	 location.href ="noticeUpdateForm.do?cp=${param.cp}&no=${param.no}${searchStr}";		  
