...
<div id="wrapper">
    ...
    <div id="wrapper2">
      ...
      <form action="${contextPath}/qna/qnaUpdate.do?cp=${param.cp}&no=${param.no}${searchStr}" method="post"
     			 enctype="multipart/form-data" role="form" onsubmit="return qnaUpdateValidate();"	 >
        <div>
            <h3 style=" margin-left:10px;">Q&A 수정</h3>
            <hr>
        </div>
        <div>
            <input type="text" id="qnaTitle" name="qnaTitle"style="width:80%; border:none;margin-left:10px;"  placeholder="제목을입력하세요."value="${qna.qnaTitle}">
            <hr>
        </div>
        <div>
        ...
        <hr>
         <p style="color:gray; margin-left:10px;">글 공개 여부</p>
           <div style="margin-left:10px;">
            <label for="private" >비공개</label>
            <input type="radio" id="private" value="Y" name="publicCheck" <c:if test="${qna.qnaPrivate =='Y'}">checked</c:if>>
            
            <label for="public">공개</label>
            <input type="radio" id="public" value="N" name="publicCheck" <c:if test="${qna.qnaPrivate =='N'}">checked</c:if>>
       	</div>
        <hr>
          <p style="color:gray; margin-left:10px;">문의내용</p>
          <textarea rows="10" id="qnaContent" name="qnaContent"style="resize: none; width: 890px; border: 3px solid #8bd2d6;
   								 border-radius: 5px; margin-left:10px;">${qna.qnaContent}</textarea> 
        ...
        <div class="float-right" style="margin-left:10px;">
            ...
            <button type="button"  class="btn"  style="background-color:#8bd2d6; color:white;"  onclick="history.back()">취소</button>
        </div>
...
$(function(){
	  $("#qnaFileArea").hide(); // #fileArea 요소를 숨김,
...
$(".boardImg").eq(${file.qnaImgLevel}).children("img")
.attr("src","${contextPath}/resources/image/center/qnaImg/${file.qnaImgName}");
